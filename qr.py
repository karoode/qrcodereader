# qr.py — Visitor QR & Badge server
# Portrait badge (Times New Roman Bold 48, 1.5 line spacing, left alignment, wrap long values)
# NEW:
#   • QR صغير وأنيق بأسفل البطاقة (visitor_id فقط) — بالمنتصف + مسافة أمان عالية
#   • landscape-trick: نسخة مقلوبة 90° لليسار للطابعات التي تجبر Landscape
# Endpoints:
#   - /card/<vid>.png               → Portrait مع QR بالأسفل
#   - /card_landscape/<vid>.png     → نفس البطاقة لكن مقلوبة 90° لليسار
#   - /decode_badge[...] (POST)     → يرجّع افتراضياً النسخة المقلوبة 90° (يمكن تعطيلها بـ ?landscape_trick=0)

import os, io, sqlite3, string, secrets, base64, mimetypes
...
def wrap_lines(draw, text, font, max_w):
    """يلفّ النص لأسطر متعددة بدون تصغير حجم الخط."""
    text = (text or "").strip()
    if not text:
        return [""]
    words = text.split()
...
def compose_badge_portrait(name, company, position, visitor_id=None, w=BADGE_W, h=BADGE_H):
    ...
    # Logo أعلى — مرفوع لفوق بمقدار LOGO_SHIFT_UP_CM دون تجاوز أعلى الصفحة
...
<style>
  :root{
    --bg:#ffffff; --fg:#0f141a; --muted:#6b7280; --border:#e5e7eb; --btn:#111827; --btnfg:#ffffff;
  }
  *{box-sizing:border-box}
  ...
  .logo-fixed{
    position:fixed; right:24px; top:24px; width:120px; height:auto; opacity:.9;
  ...
  button{
    font-size:16px;border-radius:10px;border:1px solid #111827;background:var(--btn);color:var(--btnfg);
    padding:12px 16px;cursor:pointer
  }
  .stack{display:flex;flex-direction:column;gap:12px} /* إدخال بالطول */
...
@app.route("/ui_logo")
def ui_logo():
    """Serve the same badge logo in the UI (right-fixed)."""
    if not os.path.exists(LOGO_PATH):
        # 1×1 أبيض إذا الشعار غير موجود
        img = Image.new("RGB", (1,1), "white")
        buf = io.BytesIO(); img.save(buf, "PNG"); buf.seek(0)
        return send_file(buf, mimetype="image/png")
    # حاول تخمين المايم تايب من الامتداد
...
def create():
    ...
    # نعرض فقط QR + الاسم (لا نعرض الـPIN ولا أزرار الباج)
    return render_template_string(INDEX_HTML, title=APP_TITLE, rec=rec)

@app.route("/qr/<vid>.png")
def qr_png(vid):
    """إرجاع QR كصورة؛ يدعم التنزيل المباشر إذا تم تمرير ?dl=1"""
    ensure_db()
    with sqlite3.connect(DB_PATH) as con:
        con.row_factory = sqlite3.Row
        cur = con.cursor()
...
# Upload form (GET) for manual testing
...
# Flexible decode: /decode_badge, /decode_badge/, /decode_badge/decode, ...
